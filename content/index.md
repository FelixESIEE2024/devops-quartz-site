---
title: Home
publish: true
---
# ######### ajout - Page d'accueil personnalisée avec hello felix
# Ce fichier README.md est converti en page d'accueil (content/index.md) par le script

# Bienvenue sur le site DevOps - Felix 👋

## Hello Felix! 

Ce site présente les projets et laboratoires DevOps de l'équipe ESIEE.

---

## 👥 **Auteurs**

- **CONQUI Félix**
- **Thomas ASHAR ALI**

---

## 📚 **Laboratoires et Ressources**

### 🏗️ Lab 2: Managing Infrastructure as Code (IaC)

Apprenez à gérer votre infrastructure en tant que code avec les meilleures pratiques DevOps.

**📖 [Accéder à Lab 2 →](https://fantasy-cyclamen-e0f.notion.site/Lab2-Managing-Infrastructure-as-Code-IaC-2c829d729aba80cb8de4e042082bd979?source=copy_link)**

---

### 🚀 Lab 3: How to Deploy Your Apps

Maîtrisez les techniques de déploiement d'applications modernes et automatisées.

**📖 [Accéder à Lab 3 →](https://fantasy-cyclamen-e0f.notion.site/Lab3-How-to-Deploy-Your-Apps-2ca29d729aba804b8820d93c9133603f?pvs=74)**

---

### ⚙️ Lab 5: Configuration Avancée

Explorlez les configurations avancées et les cas d'usage complexes.

**📖 [Accéder à Lab 5 →](https://fantasy-cyclamen-e0f.notion.site/Lab-5-VRAI-FICHIER-2e529d729aba8157b6e4efed27fe2ec8?source=copy_link)**

---

## 🌐 **Informations du Déploiement**

| Élément | Valeur |
|---------|--------|
| **Générateur** | Quartz v4 |
| **Hébergement** | Cloudflare Pages |
| **Protection** | Cloudflare Access (emails @esiee.fr) |
| **Projet** | felix-devops-site |
| **URL** | https://felix-devops-site.pages.dev |

---

## 📋 **Contenu Disponible**

- ✅ Documentation et guides DevOps
- ✅ Ressources Infrastructure as Code
- ✅ Tutoriels de déploiement
- ✅ Intégration continue et déploiement continu (CI/CD)

---

## 🔐 **Sécurité**

Ce site est protégé par **Cloudflare Access** et réservé aux utilisateurs avec des adresses email autorisées (@esiee.fr, @edu.esiee.fr).

---

**Dernière mise à jour** : 20 janvier 2026 | **Statut** : ✅ En ligne et opérationnel
